...
É um patrocinador que participa da associação como um membro (associado), ele pode usufruir dos benefícios oferecidos pela associação.
...
### Usuário - Caso de Uso
- Usuário não deve ser capaz de acessar o sistema caso o `is_account_active` = `false`
...
- ADM deve ser capaz de cadastrar benefícios
  - Benefícios não precisam estar vinculado a um `sponsor`
    - Estes sempre estarão visíveis para os usuários até que o ADM desabilite-o
  - Benefícios vinculados a `sponsor` só devem ser exibidos se o patrocinador estiver ativo
    - Ao desativar um `sponsor` os benefícios associados não devem ser visíveis
